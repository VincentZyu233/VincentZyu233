...
Windows 下 PowerShell 的 `Move-Item` / `Remove-Item -Recurse` 遇到带 `.git` 的目录时，有时会因为隐藏、只读属性或文件占用报错：
...
3. 再执行同样的 `cmd /c ren ...` 或 `cmd /c rmdir /s /q ...` 命令。
...
`rmdir /s /q` 是强删除命令，不会进入回收站。执行前务必确认路径正确，尤其不要在不确定当前位置时删除相对路径。
